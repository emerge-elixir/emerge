...
waits requires versioned edit/echo authority and batched publication. Input/output
queues remain unbounded.
...
Terminal overflow failure and default limits have not been approved. Spare FIFO
capacity persists during partial replay and is released synchronously on full drain
or runtime destruction; coalescing alone is not an overflow policy. A hard limit
requires explicit backpressure/overflow semantics that preserve control/Stop
independently, not silent input eviction.
