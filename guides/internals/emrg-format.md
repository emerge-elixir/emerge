...
nearby_mask       # 1 byte bitmask
nearby_ids...     # repeated mounted nearby ids in fixed slot order
```

Nearby slot order for `nearby_mask` bits and encoded nearby ids is:
...
Only set bits have an accompanying nearby id payload.
...
Rust still accepts EMRG v3 during decode for compatibility with older direct
tree-upload tests, but all new Elixir serialization uses v4.
