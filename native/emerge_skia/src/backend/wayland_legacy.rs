//! Legacy Wayland/X11 backend using winit and glutin.
//!
...
impl LegacyWake {
    fn new(proxy: EventLoopProxy<UserEvent>) -> Self {
        Self {
            proxy: Arc::new(Mutex::new(Some(proxy))),
        }
    }
}

impl BackendWake for LegacyWake {
    fn request_stop(&self) {
        let proxy = match self.proxy.lock() {
...
pub fn run(
    ...
    let wake = BackendWakeHandle::new(LegacyWake::new(el.create_proxy()));
