...
pub struct RendererStatsSnapshot {
    ...
    pub patch_tree_total: DurationStatsSnapshot,
}
...
struct RendererStatsWindow {
    ...
    patch_tree_total: DurationStatsWindow,
}
...
impl RendererStatsWindow {
    fn new(started_at: Instant, last_display_interval_ns: Option<u64>) -> Self {
        Self {
            ...
            patch_tree_total: DurationStatsWindow::default(),
        }
    }
}
...
impl RendererStatsCollector {
    ...
    pub fn record_patch_tree_total(&self, duration: Duration) {
        self.record_duration(duration, |window| &mut window.patch_tree_total);
    }
    ...
    pub fn snapshot(&self) -> RendererStatsSnapshot {
        ...
        let snapshot = RendererStatsSnapshot {
            ...
            patch_tree_total: window.patch_tree_total.snapshot(),
        };
...
pub fn format_renderer_stats_log(backend_label: &str, snapshot: &RendererStatsSnapshot) -> String {
    format!(
        concat!(
            ...
            "patch_tree_actor_total_ms_avg={:.3} patch_tree_actor_total_ms_min={:.3} ",
            "patch_tree_actor_total_ms_max={:.3} patch_tree_actor_total_ms_count={}"
        ),
        backend_label,
        ...
        snapshot.patch_tree_total.avg_ms,
        snapshot.patch_tree_total.min_ms,
        snapshot.patch_tree_total.max_ms,
        snapshot.patch_tree_total.count,
    )
}
...
mod tests {
    ...
    fn snapshot_tracks_avg_min_max_and_resets_window() {
        ...
        stats.record_patch_tree_total(Duration::from_millis(9));
        ...
        assert_eq!(snapshot.patch_tree_total.count, 1);
        ...
        assert_eq!(reset_snapshot.patch_tree_total.count, 0);
    }
    ...
    fn log_format_includes_all_stats_fields() {
        ...
        stats.record_patch_tree_total(Duration::from_millis(7));
        ...
        assert!(message.contains("patch_tree_actor_total_ms_count=1"));
    }
}
