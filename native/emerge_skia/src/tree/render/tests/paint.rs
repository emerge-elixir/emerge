...
fn test_render_scrollbar_y_thumb() {
    ...
    let background = only_draw(&draws, |draw| {
        matches!(
            draw.primitive,
            DrawPrimitive::RoundedRect(0.0, 0.0, 100.0, 50.0, 8.0, 0x000000FF)
        )
    });
...
fn test_render_scrollbar_x_thumb() {
    ...
    only_draw(&draws, |draw| {
        matches!(
            draw.primitive,
            DrawPrimitive::RoundedRectCorners(
                0.0, 0.0, 80.0, 40.0, 4.0, 6.0, 12.0, 8.0, 0x000000FF
            )
        )
    });
...
fn test_render_gradient_with_rounded_corners_emits_radius() {
    ...
    match gradient.primitive {
        DrawPrimitive::Gradient(_, _, _, _, _, _, _, radius) => {
